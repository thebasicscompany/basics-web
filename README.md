...
- **apps/agent** — LiveKit Agents (Node.js) voice tutor worker. STT/LLM/TTS pipeline with tools for whiteboard drawing, teaching state, mastery, and checkpoints. Persists everything to the session event log and broadcasts events to the room over data channels.
...
- **packages/tutor** — text tutoring runtime (`TutorRuntime` interface, AI SDK implementation) used by the NDJSON turn API.
...
pnpm db:migrate
pnpm db:seed
pnpm --filter @basics/agent download-files   # one-time: VAD + turn-detector models
...
- AI SDK types never leave `packages/tutor`; LiveKit agent types never leave `apps/agent`. The UI and API consume only contract types.
