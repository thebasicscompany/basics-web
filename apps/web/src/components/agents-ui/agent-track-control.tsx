...
function TrackDeviceSelect({
  ...
  return (
    <Select
      ...
      onValueChange={(value) => {
        // Base UI's Select can emit null; ignore cleared values.
        if (value) handleActiveDeviceChange(value);
      }}
    >
      ...
      {/* Base UI equivalent of the registry's Radix `position="popper"`: open as a
          popup anchored to the trigger and size to content instead of overlaying
          the selected item on top of the trigger (macOS-native behavior). */}
      <SelectContent alignItemWithTrigger={false} align="start" className="w-max">
